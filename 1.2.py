...
def get_split_next_moves(G, current_node, method, shortest_dists,
                         score_ratio=1.15, score_slack=2.0, min_split_share=0.05):
    """
    改进算法的方案 3：
    - 不限制最大分支数
    - 只保留和最优路径代价足够接近的候选路
    ...
    picked = [
        ...
        if item[1] <= best_cost * score_ratio or item[1] <= best_cost + score_slack
    ]
    ...
    capacity_left = {v: G[current_node][v]["capacity"] * DELTA_T for v, _ in picked}
    ...
    while remaining > 1e-9 and active:
        ...
        filtered = [v for v in active if proposed[v] >= min_allowed]
        if not filtered:
            filtered = [min(active, key=lambda v: score_map[v])]
...
def _process_transit_arrivals(G, current_time, evacuated_by_line=None, exit_usage_dict=None):
...
def _schedule_moves_as_transit(G, moves):
    ...
    for u, v, amount in moves:
        ...
        for line_id, ppl in G.nodes[u].get("people_dict", {}).items():
            ...
            line_shares[line_id] = line_shares.get(line_id, 0.0) + flow
...
def _select_critical_queue_node(G, metrics, line_id):
    ...
    for node in metrics.get("node_series", {}).keys():
        if not _is_queue_node(G, node):
            continue
        if line_id not in _infer_node_line_ids(G, node):
            continue
        ...
        for line_id, target in target_by_line.items():
            if target > 0 and metrics["clearance_times_by_line"].get(line_id) is None:
                if evacuated_by_line[line_id] >= target * 0.99:
                    metrics["clearance_times_by_line"][line_id] = current_time + DELTA_T
...
def plot_detailed_dynamics_by_line(G_base, metrics_trad, metrics_imp, pop_dict):
    ...
    for idx, line in enumerate(lines):
        ...
        ax.plot(times_imp, q_imp, 'g-', linewidth=2.2, label="Improved 队列长度")
        ax2.plot(times_trad, occ_trad, 'r--', linewidth=1.8, label="Traditional 占用率", alpha=0.65)
        ax2.plot(times_imp, occ_imp, 'g--', linewidth=1.8, label="Improved 占用率")
    ...
    for idx, line in enumerate(lines):
        ...
        ax.plot(times_imp, travel_imp, 'b-', linewidth=2.8, label="Improved (改进)")
...
if __name__ == "__main__":
    ...
    print("\n📊 正在生成各线路关键瓶颈对比图 (排队长度、实际旅行时间)...")
    plot_detailed_dynamics_by_line(G, metrics_trad, metrics_imp, current_pop_dict)
