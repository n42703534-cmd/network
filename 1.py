...
def init_people(G, pop_dict, apply_noise=False, rng=None):
    ...
    for line_id, spec in PLATFORM_VERTICAL_SPECS.items():
        ...
        if p_node in G.nodes and line_id in pop_dict:
            ...
            # 4. 放入【换乘通道区】(按你给的换乘数字，洒在包含该线名字的换乘通道里)
...
def run_simulation_for_metrics(G_base, pop_dict, method="Improved", apply_noise=False, rng=None):
    ...
    metrics = {
        "time": 0, "queueing_time": 0.0, "congestion_exposure_time": 0.0, "avg_speed": 0.0,
        ...
        "node_stats": {n: {"peak_people": 0.0, "peak_density": 0.0, "queue_seconds": 0.0, "congestion_seconds": 0.0} for
                       n in G.nodes()},
        # 🌟 核心新增：记录每一条线、每一秒钟的瞬时细节！
        "time_series_by_line": {line: {"times": [], "speeds": [], "queues": []} for line in ALL_LINE_IDS}
    }
    ...
    while time < 6000:
        ...
        for l, target in target_by_line.items():
            if target > 0 and evacuated_by_line[l] < target * 0.99:
                ...
                break
        if all_lines_cleared: break

        moves = []
        active_nodes = [n for n in G.nodes() if G.nodes[n].get("people", 0) > 0.1 and G.nodes[n].get("type") != "exit"]

        current_heavy_congestion = 0
        ...
        if monitor_node and monitor_node in G.nodes: metrics["time_series_queue"][monitor_node].append(
            G.nodes[monitor_node].get("people", 0))
        ...
        for u, v, amount in moves:
            ...
            if G.nodes[v].get("type") == "exit": metrics["exit_usage"][v] += amount
...
def run_simulation_for_metrics(G_base, pop_dict, method="Improved", apply_noise=False, rng=None):
    ...
    metrics = {
        ...
        "clearance_times_by_line": {line: None for line, pop in target_by_line.items() if pop > 0},
        "queueing_time_by_line": {line: 0.0 for line in ALL_LINE_IDS},
        "congestion_exposure_by_line": {line: 0.0 for line in ALL_LINE_IDS},  # 🌟 新增：各线路拥挤暴露统计
        "exit_usage_by_line": {ext: {line: 0.0 for line in ALL_LINE_IDS} for ext in
                               NODES_DATA.get("ALL_EXITS", {}).keys()},
        "exit_usage": {ext: 0 for ext in NODES_DATA.get("ALL_EXITS", {}).keys()},
        "time_series_queue": {monitor_node: []} if monitor_node else {},
    ...
    while time < 6000:
        ...
        evacuated_count += execute_moves(G, moves, evacuated_by_line, metrics["exit_usage_by_line"])
...
if __name__ == "__main__":
    ...
    REAL_POPULATIONS = {
        "L2": {
            "train_1": 2400, "train_2": 2400,
            "platform_waiting": 236,
            "hall_people": 350,  # 2号线站厅
            "transfer_people": 526  # 2号线换乘中
        },
        "L7": {
            "train_1": 1620, "train_2": 1620,
            "platform_waiting": 219,
            "hall_people": 112,  # 7号线站厅
            "transfer_people": 169  # 7号线换乘中
        },
        "L16": {
            "train_1": 1230, "train_2": 1230,
            "platform_waiting": 42,
            "hall_people": 15,  # 16号线站厅
            "transfer_people": 27  # 16号线换乘中
        },
        "L18": {
            "train_1": 1665, "train_2": 1665,
            "platform_waiting": 178,
            "hall_people": 125,  # 18号线站厅
            "transfer_people": 188  # 18号线换乘中
        },
        "Maglev": {
            "train_1": 959, "train_2": 959,
            "platform_waiting": 0,
            "hall_people": 0,
            "transfer_people": 0
        }
    ...
    for line, data in REAL_POPULATIONS.items():
        ...
        if mode == 1:
            ...
            t1, t2 = int(data["train_1"]), 0
        elif mode == 3:
            t1, t2 = 0, int(data["train_2"])
        elif mode == 4:
            t1, t2 = int(data["train_1"]), int(data["train_2"])

        # 站台、站厅、换乘是基础客流，永远都在
        pw = int(data["platform_waiting"])
        hp = int(data["hall_people"])
        tp = int(data["transfer_people"])
